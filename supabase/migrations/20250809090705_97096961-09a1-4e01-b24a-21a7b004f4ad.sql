-- Fix enum casts in generate_training_plan to avoid syntax errors
  ...
  FOR week_num IN 1..(base_weeks + build_weeks + peak_weeks + taper_weeks) LOOP
    ...
    LOOP
      ...
      CASE workout_type_assigned
        WHEN 'Easy', 'Recovery' THEN workout_intensity := 'Low'::intensity_type;
        WHEN 'Tempo', 'Hill' THEN workout_intensity := 'Moderate'::intensity_type;
        WHEN 'Interval', 'Long' THEN workout_intensity := 'High'::intensity_type;
        ELSE workout_intensity := 'Low'::intensity_type;
      END CASE;
